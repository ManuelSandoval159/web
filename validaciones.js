...
document.getElementById('password').addEventListener('input', function() {
    ...
    emailRegex = /^[a-zA-Z0-9]{5,8}$/i;
    if (emailRegex.test(campo.value)) {
        valido.innerText = "";
        document.getElementById('labelContra').style.color =  "#80868B";//Contrasena
        
    } else {
    valido.innerText = "Password a-zZ-a0-9 5-8";
...
document.getElementById('celular').addEventListener('input', function() {
    campo = event.target;
    emailRegex = /^[0-9 \s]{10}$/i;
    if (emailRegex.test(campo.value)) {
        document.getElementById('labelCelular').style.color =  "#80868B";//celular
    } else {
        document.getElementById('labelCelular').style.color =  "#e81a1a";
    }
}); 
